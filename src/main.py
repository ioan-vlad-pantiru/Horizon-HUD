...
from src.perception.corridor import build_corridor_polygon, draw_corridor
...
def _build_pipeline(cfg: dict[str, Any]) -> tuple:
    ...
    return detector, tracker, imu, orient_est, compensator, risk_engine
...
def main() -> None:
    ...
    detector, tracker, imu, orient_est, compensator, risk_engine = _build_pipeline(cfg)

    # ── open video source ──────────────────────────────────────────────────────
    is_webcam = args.source.lower() == "webcam"
    ...
    logger.info("Video source opened: %s", args.source)
    ...
    # Corridor polygon is rebuilt on first frame when frame_size is known;
    # force a rebuild after first real frame:
    ...
    try:
        while True:
            ...
            # ── corridor polygon (stable after first frame) ───────────────────
            if corridor_poly is None or corridor_poly[0, 1] != fh:
                corridor_cfg = corridor_config_from_dict(cfg.get("corridor", {}))
            ...
            _draw_status(
                frame, orient,
                compensator.enabled, imu.scenario.name, detector_name,
